...
## Project
Tremend scraper for peviitor.ro (Node.js, ESM, Jest) — derived from the [EPAM nodejs template](https://github.com/sebiboga/epam-systems-international-srl-nodejs-scraper).

## 📐 This Repo Is a Derived Scraper
This repo is derived from the **EPAM reference implementation** for Node.js scrapers in the peviitor.ro ecosystem. The template lives at [sebiboga/epam-systems-international-srl-nodejs-scraper](https://github.com/sebiboga/epam-systems-international-srl-nodejs-scraper).
...
- **Only the scraping logic in `scraper/index.js`** (`fetchJobsPage`, `parseHtmlJobs`) is Tremend-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/tremendous-software-consulting-srl-nodejs-scraper --json status -q .status
```
...
### 6. DO NOT modify these files (inherited from template)
- `scraper/anaf.js`
- `scraper/company.js`
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo peviitor-scrapers/tremendous-software-consulting-srl-nodejs-scraper --state open`
