...
This scraper extracts job listings from the Tremend careers page (WordPress AJAX) and imports them to peviitor.ro.

Target: https://tremend.com/career/
...
1. **Start with brand** - We know the brand ("Tremend")
...
8. **Scrape new jobs** - Query the WordPress AJAX endpoint (`https://tremend.com/wp-admin/admin-ajax.php`, `action=career_filter`, paginated) and parse the `career__job-card` HTML; keep only Romanian locations
...
> **Important**: Scraper does NOT delete jobs from other sources. It only upserts Tremend careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Fetch Tremend careers pages via WordPress AJAX (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape Tremend careers (WordPress AJAX, jobs for Romania)
    │
    ▼
...
- **Tremend Careers (WordPress AJAX)**: `https://tremend.com/wp-admin/admin-ajax.php` — POST `action=career_filter` + `paged`, header `X-Requested-With: XMLHttpRequest`; returns `{success, data: {html, found_posts, max_num_pages}}`
...
- [x] Write Unit/Component/E2E tests for index.js (Tremend parseHtmlJobs)
