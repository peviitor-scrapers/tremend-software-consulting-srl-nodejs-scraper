...
const baseCompany = {
  ...
  career: ["https://tremend.com/career/"],
  lastScraped: "2026-06-17"
};

const baseJob = {
  url: "https://tremend.com/career/123_en",
...
describe("generateJobsMarkdown", () => {
  describe("company section", () => {
    it("includes company name as h1", () => {
      ...
      expect(md).toContain("# TREMEND SOFTWARE CONSULTING SRL");
    });

    it("includes CIF", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("18089451");
    });

    it("includes brand", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("Tremend");
    });
    ...
    it("includes website as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("[https://tremend.com](https://tremend.com)");
    });
    ...
    it("omits optional fields when not present", () => {
      const minimal = { id: "18089451", company: "TREMEND SOFTWARE CONSULTING SRL" };
  ...
  describe("jobs section", () => {
    ...
    it("includes job URL as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, [baseJob]);
      expect(md).toContain("[https://tremend.com/career/123_en]");
    });
    ...
    it("renders multiple jobs", () => {
      const job2 = { ...baseJob, title: "DevOps Engineer", url: "https://tremend.com/career/456_en" };
    ...
    it("handles job with no optional fields", () => {
      const minimal = { url: "https://tremend.com/career/999_en", title: "QA Engineer" };
